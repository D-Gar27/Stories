  ...
  useEffect(() => {
    ...
    if (router.isReady) {
      (async () => {
        ...
        if (isMounted) {
          setStory(data);
          setToEdit({ title: data.title, textBody: data.textBody });
        }
      })();
    }
      ...
      <section className="w-full max-w-xl md:max-w-2xl xl:max-w-4xl mx-auto mt-8 px-3">
        ...
        <div className="flex w-full justify-between items-center mt-4">
          <div className="flex items-center justify-start gap-2">
            ...
            <p className="text-sm text-gray-700 font-bold">{story?.username}</p>
          </div>
          <Moment fromNow>{story?.timestamp.toDate()}</Moment>
        </div>
        {editing ? (
          <input
            type="text"
        ...
        {editing ? (
          <textarea
            ...
            onChange={(e) => setToEdit({ ...toEdit, textBody: e.target.value })}
            name=""
